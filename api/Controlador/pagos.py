...
class PagoControl(View):
    ...
    def post(self, request):
        ...
        PagosModelo.objects.create(nombre=pago['nombre'], descripcion=pago['descripcion'], costo=pago['costo'], aplicacion_id=pago['aplicacion_id'])
        return JsonResponse(datos)
    ...
    def put(self, request, id):
        ...
        if len(Existepago)>0:
                ...
                pago.costo = jd['costo']
                pago.aplicacion_id = jd['aplicacion_id']
                pago.save() 
                datos ={'message': "Success"}
        else:
                datos ={'message': "Pago no enconctrado"}
